...
idx2=find(spcs2num==2); %versicolor만찾기
idx3=find(spcs2num==3); %virginica만 찾기


figure;
plot(meas(idx1,1),meas(idx1,2),'r.'); hold on; %setosa는 빨간색점으로
plot(meas(idx2,1),meas(idx2,2),'go'); hold on; %versicolor는 녹색 동그라미로
plot(meas(idx3,1),meas(idx3,2),'bx'); hold on; %virginica는 파란색x로
xlabel('Sepal length');
ylabel('Sepal width');
...
%오늘은 편의상 아래와 같이 나누겠음
%1~50: setosa, 51~100: versicolor, 101~140: virginica
